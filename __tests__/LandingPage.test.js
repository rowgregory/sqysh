...
jest.mock('@fortawesome/react-fontawesome', () => ({
  FontAwesomeIcon: ({ icon, color, className, size }) => (
    <svg data-testid={`icon-${icon.iconName}`} style={{ color }} className={className} />
  ),
}));
...
describe('Banner Component', () => {
  ...
  it('should render the main heading', () => {
    const mainHeading = screen.getByRole('heading', { name: /transform your digital future with sqysh/i });
    expect(mainHeading).toBeInTheDocument();
  });

  it('should render the description paragraph', () => {
    const descriptionParagraph = screen.getByText(/next-gen software solutions: modular and scalable/i);
...
describe('Journey Component', () => {
  ...
  it('should render the main heading', () => {
    const mainHeading = screen.getByRole('heading', { name: /my journey and mission/i });
    expect(mainHeading).toBeInTheDocument();
  });

  it('should render the main paragraph', () => {
    const mainParagraph = screen.getByText(/committed to delivering exceptional software solutions that empower businesses to achieve their full potential through innovation and expertise/i);
    expect(mainParagraph).toBeInTheDocument();
  });

  it('should render the "From Vision to Code" article', () => {
    const visionHeading = screen.getByRole('heading', { name: /from vision to code/i });
    expect(visionHeading).toBeInTheDocument();
    const visionParagraph = screen.getByText(/turning your ideas into custom software solutions/i);
    expect(visionParagraph).toBeInTheDocument();
  });

  it('should render the "Automated Workflow Processes" article', () => {
    const workflowHeading = screen.getByRole('heading', { name: /automated workflow processes/i });
    expect(workflowHeading).toBeInTheDocument();
    const workflowParagraph = screen.getByText(/designing custom software solutions that automate repetitive tasks, optimize operations, and boost efficiency/i);
